...
// Pretpostavka: u sesiji imaš userid i name
$currentUserID = $_SESSION['userid'] ?? null;
$currentUserName = $_SESSION['username'] ?? '';

// Ako nema usera u sesiji, prekini
if (!$currentUserID) {
    die("Not logged in");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Owner je ulogovani user
    $owner = $currentUserID;

    if (empty($shortDesc) || empty($desc) || empty($group) || empty($sector) ||
        empty($category) || empty($subcategory) || empty($owner) || empty($assignedOn) || empty($status)) {
            ...
            <div class="flex-row">
                <fieldset class="fieldset">
                    ...
                    <div class="form-group">
                        ...
                        <input type="hidden" name="owner" value="<?= htmlspecialchars($currentUserID); ?>">
                    </div>
                </fieldset>
            </div>
            <button type="submit">Create ticket</button>
        </form>
        <a href="index-user.php" class="back-link">&#8592; Back to dashboard</a>
    </div>
